...
# /home/NFS/users/l.pagie/usr/local/src/miniconda2/bin/samtools mpileup -t AD -v -d 50 -u -r 1:21362561-21362633 -f /home/NFS/users/c.leemans/data/GRCm38/GRCm38_CAST.fa results/sorted/CM1414_fwd.bam

pileup_format = "{} mpileup -t AD -v -d 50 -u -r {}:{}-{} -f {} {}"
awk = ('BEGIN {m=0}'
       '{'
       '    if ($1 ~ /^[^#]/){'
       '        match($8, "DP=([0-9]+).*QS=([0-9.]*)",arr); '
       '        if (arr[2] < 0.5 && arr[1] > 1){'
       '            m += 1'
       '        }'
       '    }'
       '}'
