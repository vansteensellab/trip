...
P$class.GROcap[NREP]<-"escaper"
P$class.GROcap[REP]<-"repressed"
P$class.GROcap[BND] <- "boundary"
P$class.GROcap = factor(P$class.GROcap, levels=c('iLAD', 'escaper', 'repressed', 'inactive', 'boundary'))
...
p_classes = P[which(P$class.GROcap %in% c('inactive', 'escaper', 'repressed')),]
ggplot(P, aes(x=SuRE, y=GROcap)) + 
    geom_point(size=0.1,color=COLi) + 
    geom_point(data=p_classes, aes(color=factor(class.GROcap, levels=c('inactive', 'repressed', 'escaper'))), size=0.6) + 
...
REP<- P$SuRE> 0.3 & P$LRS< -1 & P$LAD==1  & P$CAGE< -1 #repressed
Pcnts<-c(length(which(REP)), length(which(NREP)), length(which(INACT)))
names(Pcnts)<-c("repressed", "escaper", "inactive")
BND <- P$LAD==1 & !INACT & !NREP & !REP
...
P$class.CAGE[NREP]<-"escaper"
P$class.CAGE[REP]<-"repressed"
P$class.CAGE[BND] <- "boundary"
P$class.CAGE = factor(P$class.CAGE, levels=c('iLAD', 'escaper', 'repressed', 'inactive', 'boundary'))
...
ggplot(P, aes(x=SuRE, y=CAGE, color=ifelse(LAD==1, 'LAD', 'iLAD'))) +
    ...
    theme(legend.title=element_blank()) +
    scale_color_manual(values=COL_lad)
p_classes = P[which(P$class.CAGE %in% c('inactive', 'escaper', 'repressed')),]
ggplot(P, aes(x=SuRE, y=CAGE)) + 
    geom_point(size=0.1,color=COLi) + 
    geom_point(data=p_classes, aes(color=factor(class.CAGE, levels=c('inactive', 'repressed', 'escaper'))), size=0.6) + 
...
ggplot(P, aes(x=SuRE, y=GROcap)) + 
    geom_point(size=0.1,color=COLi) + 
    geom_point(data=p_classes, aes(color=factor(class, levels=c('inactive', 'repressed', 'escaper'))), size=0.6) + 
    geom_line(data=RM, aes(x=SuRE.mean, y=GROcap.ilad), color=COL_lad['iLAD']) +
    theme(legend.title=element_blank()) +
    scale_colour_manual(values=COL_class)
...
this_p = P[P$class%in% c('iLAD', 'escaper'), ]
...
s_list = list(iLAD=this_p$name[s_ilad], escaper=this_p$name[s_escaper])
for (dam in c('lamina', 'nucleolus', 'centromere', 'speckles', 'pores')){
    plot_list = list()
    for (i in c('iLAD', 'escaper')){
        s<-s_list[[i]]
        w<-oENST %in% s #which rows in oENST correspond to genes in s
        subPOS<-oPOS[w]
        subDamFus <-damid[w, paste0('K562.',dam)]
...
count_list = lapply(count_file_vec, function(x){
        ...
        count_table = count_table[,c(1:3, 12)]
...
P$tssr_proseq_sense[P$strand=='+'] = score_table[P$strand=='+', 'tssr_PROseq_plus']
P$tssr_proseq_sense[P$strand=='-'] = score_table[P$strand=='-', 'tssr_PROseq_minus']
P$tssr_proseq_antisense[P$strand=='+'] = score_table[P$strand=='+', 'tssr_PROseq_minus']
P$tssr_proseq_antisense[P$strand=='-'] = score_table[P$strand=='-', 'tssr_PROseq_plus']
P$body_proseq_sense[P$strand=='+'] = score_table[P$strand=='+', 'body_PROseq_plus']
P$body_proseq_sense[P$strand=='-'] = score_table[P$strand=='-', 'body_PROseq_minus']
P$body_proseq_antisense[P$strand=='+'] = score_table[P$strand=='+', 'body_PROseq_minus']
P$body_proseq_antisense[P$strand=='-'] = score_table[P$strand=='-', 'body_PROseq_plus']
this_p = P[!P$class%in%c('boundary', 'not_sure'),]
...
this_p = P[P$class%in%c('escaper', 'iLAD'), ]
s_escaper = which(this_p$class=='escaper')
gro_order = order(this_p$GROcap)
gro_escaper = which(gro_order %in% s_escaper)
...
for(i in c('iLAD', 'escaper', 'repressed', 'inactive')) #for the three gene classes
   {
    ...
    median_list = lapply(seq(1,440, 8), function(x){
                           rowm = rowMeans(plus[,x:(x+7)])
                           quant = quantile(rowm, probs=c(0.025, 0.975))
                           median = median(rowm)
                           c(x*100+300, median, quant)
                       })
    median = do.call(rbind.data.frame, median_list)
    colnames(median) = c('x', 'y', 'ymin', 'ymax')

    plot_list[[i]] = ggplot(median, aes(x=x-F_window, y=y, ymin=ymin, ymax=ymax)) +
                        geom_point(col=COL_class[i]) + 
                        geom_errorbar(col=COL_class[i]) + 
                        ylim(-1,10) +
